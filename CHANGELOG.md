...
- **Lumen now says when a trusted certificate has gone unused.** Removing the CA only helps someone who thinks to press the button, and a user who stops using proxy mode never will — which is precisely how a local root outlives its purpose in a keychain. The daemon records when it last captured a call, so once a trusted CA has seen nothing for ninety days the certificate panel says so and points at the removal. A CA that was trusted and *never* used counts too: with no capture to measure from, the window runs from whichever is later, the certificate's creation or the point tracking began. That distinction is what keeps an upgrade honest — measuring from the certificate alone would have told every existing user with an older root that nothing had been captured in years, on first launch, however heavily they use it. The threshold lives in the daemon and is reported to the app, so there is no second copy of the number to drift, and an app talking to an older daemon that sends nothing simply shows no advisory. The timestamp is written to `~/.lumen/activity.json` at most once every fifteen minutes, so an intercepted call does not mean a disk write.

### Fixed

- **Removing Lumen no longer leaves its CA trusted indefinitely.** Trust set with `add-trusted-cert` outlives the application, and nothing in the app could take it back out — so deleting Lumen left a ten-year root in the login keychain, and the Windows script had reverted its own trust on `-Cleanup` all along while macOS had no equivalent at all. **Settings → Certificate → Remove Certificate** now drops the keychain trust setting and deletes `~/.lumen/ca.pem` and `ca-key.pem`. It deletes in a loop, because each call removes one matching certificate and the same CA can be trusted more than once. The README had also gone stale: it still described the old `sudo` install into the *System* keychain, from before the trust step moved to the login keychain to drop the administrator prompt during setup. It documents the login keychain the app actually uses now — narrower scope, no admin rights — and how to remove the certificate by hand.
